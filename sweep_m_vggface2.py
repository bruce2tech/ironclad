...
per identity increases (m = 1, 2, 3, ...), using **FaceNet (VGGFace2)**
...
Run example:
  PYTHONPATH=. python sweep_m_vggface2.py \
    ...
    --device auto \
    --out-json storage/mi_eval/sweep_vgg_m.json \
    --out-dir  storage/mi_eval/sweep_vgg_plots
"""
...
def plot_metric_vs_m(m_values: List[int], ys: List[float], metric_name: str, out_dir: Path):
    ...
    plt.title(f"VGGFace2 — {metric_name} vs m (cosine, centroids)")
    plt.tight_layout()
